...
class PointNextPartDecoder(nn.Module):
    def __init__(self,
        ...
        self.num_parts = kwargs.get('num_parts', 43)
        ...
        if self.cls_map == 'curvenet':
            ...
            self.global_conv1 = nn.Sequential(
                create_convblock1d(fp_channels[-2] * 2, 64,
                                   norm_args=None,
                                   act_args=act_args))
            # TODO: find a way to load the last number from the cfg
            skip_channels[0] += 64 + 128 + self.num_parts  # shape categories labels
