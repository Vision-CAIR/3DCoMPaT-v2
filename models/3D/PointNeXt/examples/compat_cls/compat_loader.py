...
class CompatLoader3D(Dataset):
    ...
    def __init__(self,
        ...
        self.data, self.seg, self.shape_ids, self.label = load_data(data_root, self.partition)
...
if __name__ == '__main__':
    test_loader = CompatLoader3D(data_root='../data/fine_grained', num_points=4096, split='train')
    for data, label, seg, shape_id in test_loader:
        print("The Data:",data.shape)
        print("The Label: ",label.shape)
        print("The Segment: " , seg.shape)
        print("The Shape ID: ", shape_id)
        break 

    print("Number of Classes: ", test_loader.num_classes())
    print("Number of Segments: ", test_loader.num_segments())
