...
class get_model(nn.Module):
    ...
    def forward(self, xyz, cls_label=None):
        ...
        if (self.shape_prior):
            cls_label_one_hot = cls_label.view(B,self.num_shapes,1).repeat(1,1,N)
            l0_points = self.fp1(l0_xyz, l1_xyz, torch.cat([l0_xyz,l0_points,cls_label_one_hot],1), l1_points)
        else:
