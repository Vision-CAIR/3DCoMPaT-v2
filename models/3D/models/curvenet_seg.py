...
class get_model(nn.Module):
    def __init__(self, num_classes, category=42,shape_prior=False, k=32, setting='default', npoints=2048):
        ...
        self.cic11 = CIC(npoint=npoints, radius=0.2, k=k, in_channels=additional_channel, output_channels=64, bottleneck_ratio=2, curve_config=curve_config[setting][0])
        self.cic12 = CIC(npoint=npoints, radius=0.2, k=k, in_channels=64, output_channels=64, bottleneck_ratio=4, curve_config=curve_config[setting][0])
        ...
        self.up_cic2 = CIC(npoint=npoints, radius=0.2, k=k, in_channels=(128+64+64+3) + (category if self.shape_prior else 0) , output_channels=256, bottleneck_ratio=4)
        self.up_cic1 = CIC(npoint=npoints, radius=0.2, k=k, in_channels=256, output_channels=256, bottleneck_ratio=4)
